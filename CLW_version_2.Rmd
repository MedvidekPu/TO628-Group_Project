...
#save(model_name, file="model_name.Rdata")
#load("model_name.Rdata")
...
```{r cache=TRUE, warning=FALSE}
# KNN
knn_pred <- knn(train = hotel_train[, names(hotel_train) != "is_canceled"], test = hotel_test[, names(hotel_test) != "is_canceled"], cl = hotel_train$is_canceled, k = 100) 
CrossTable(knn_pred, hotel_test$is_canceled, prop.chisq = F, prop.r=F, prop.t = F, dnn=c('Actuals','Prediction'))
kappa_knn <- confusionMatrix(as.factor(hotel_test$is_canceled), as.factor(knn_pred), positive = "1")$overall[2]
acc_knn <- confusionMatrix(as.factor(hotel_test$is_canceled), as.factor(knn_pred), positive = "1")$overall[1]
cat('KNN: ','Accuracy Ratio: ',acc_knn,'Kappa:',kappa_knn)
```

```{r cache=TRUE, warning=FALSE}
# ANN
ann <- neuralnet(is_canceled ~ . ,data = hotel_train, linear.output = F, stepmax = 1e8)
str(ann)
...
CrossTable(hotel_test$is_canceled, binary_ann[,2], prop.chisq = F, prop.r=F, prop.t = F, dnn=c('Actuals','Prediction'))
kappa_ann <- confusionMatrix(as.factor(hotel_test$is_canceled), as.factor(binary_ann[,2]), positive = "1")$overall[2]
acc_ann <- confusionMatrix(as.factor(hotel_test$is_canceled), as.factor(binary_ann[,2]), positive = "1")$overall[1]
cat('ANN: ','Accuracy Ratio: ',acc_ann,'Kappa:',kappa_ann)
```

```{r cache=TRUE, warning=FALSE}
# Decision Tree
dt <- C5.0(is_canceled ~. , data=hotel_train)
...
kappa_dt <- confusionMatrix(as.factor(hotel_test$is_canceled), as.factor(dt_pred), positive = "1")$overall[2]
acc_dt <- confusionMatrix(as.factor(hotel_test$is_canceled), as.factor(dt_pred), positive = "1")$overall[1]
...
```{r warning= FALSE}
combined <- data.frame(binary_logit,knn_pred, binary_ann[,2],dt_pred,hotel_test$is_canceled)
...
combined_model <- C5.0(is_canceled ~. , data=hotel_train_2)
...
kappa_combined <- confusionMatrix(as.factor(hotel_test_2$is_canceled), as.factor(combined_pred), positive = "1")$overall[2]
acc_combined <- confusionMatrix(as.factor(hotel_test_2$is_canceled), as.factor(combined_pred), positive = "1")$overall[1]
plot(combined_model)
